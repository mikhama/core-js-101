...
function extractNameFromTemplate(templateString) {
  const nameStartIndex = templateString.indexOf('Hello, ') + 7;
  const nameEndIndex = templateString.indexOf('!');
  const name = templateString.substring(nameStartIndex, nameEndIndex);
  return name;
}
 ...
 *   'ABABAB','BA' => 'ABAB'
 */
function removeFirstOccurrences(mainString, substring) {
...
function getRectangleString(width, height) {
  if (width <= 0 || height <= 0) {
    return 'Ширина и высота должны быть положительными числами.';
  }
  ...
  for (let i = 0; i < height - 2; i += 1) {
    result += `${verticalLine}${' '.repeat(width - 2)}${verticalLine}\n`;
  }
  result += `└${horizontalLine}┘\n`;
  return result;
}
