...
function extractNameFromTemplate(str) {
  return str.split('Hello, ')[1].split('!')[0];
}
...
function encodeToRot13(str) {
  ...
  const arr = str.split('');
  arr.forEach((el, i) => {
    if (upper.indexOf(el) >= 0) {
      const index = upper.indexOf(el);
      const targetIndex = index + 13;
      arr[i] = upper.at(targetIndex);
    }
    if (lower.indexOf(el) >= 0) {
      const index = lower.indexOf(el);
      const targetIndex = index + 13;
      arr[i] = lower.at(targetIndex);
    }
  });
  return arr.join('');
}
...
function isString(arg) {
  if (!arg) return false;
  if (typeof arg === 'object' && Object.prototype.toString.call(arg) !== '[object String]') return false;
  return true;
}
