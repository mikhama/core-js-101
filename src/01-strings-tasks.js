...
function extractEmails(str) {
  return str.split(';').length === 1 ? [str] : str.split(';');
}
...
function getRectangleString(width, height) {
  ...
  const top = leftTop + '─'.repeat(+width - 2) + rigthTop + newStr;
  const bottom = leftBottom + '─'.repeat(+width - 2) + rightBottom + newStr;
  const leftRight = side + ' '.repeat(width - 2) + side + newStr;
  const sideRepeat = leftRight.repeat(height - 2);

  return top + sideRepeat + bottom;
}
