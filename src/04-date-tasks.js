...
function timeSpanToString(startDate, endDate) {
  ...
  const milisec = String((endDate - startDate)).slice(-3);
  const str = `${hour < 10 ? `0${hour}` : hour}:${min < 10 ? `0${min}` : min}:${sec < 10 ? `0${sec}` : sec}.${milisec}`;
  return str;
}
...
function angleBetweenClockHands(date) {
  ...
  hourAngle = hour > 12 ? hourAngle - 360 : hourAngle;
